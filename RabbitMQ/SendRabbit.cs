...
namespace WebChatBot.RabbitMQ
{
    public class SendRabbit : IDisposable
    {
        ...
        public void SendMessage(string message, string messageId, string userName)
        {
            try
            {
            ...
            {
                { "UserName", userName } // Добавление имени пользователя в заголовки
            };
                _channel.BasicPublish(exchange: "", routingKey: QueueName, basicProperties: props, body: body);
                Console.WriteLine($"Sent message: {message}, MessageId: {messageId}, UserName: {userName}");
            }
            catch  (Exception ex)
            {
